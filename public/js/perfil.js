...
function renderOrdineProfilo(o) {
  const STEPS = [
    { key: 'confirmado', label: 'Confermato',    icon: '✓' },
    { key: 'confirmado', label: 'In elaborazione', icon: '⚙' },
    { key: 'enviado',    label: 'Spedito',        icon: '📦' },
    { key: 'entregado',  label: 'Consegnato',     icon: '✓' },
  ];
  ...
  const stepperHtml = o.estado === 'cancelado'
    ? `<div class="pill" style="color:var(--accent);border-color:var(--accent);margin-bottom:20px">ORDINE ANNULLATO</div>`
    ...
    ? o.detalles.map(d => `
        <div class="ocv2-item">
          ...
          <div class="ocv2-item-info">
            <div class="ocv2-item-name">${d.Prodotto?.nombre || 'Prodotto'}</div>
            ${d.variante ? `<div class="ocv2-item-meta">${d.variante}</div>` : ''}
            ${d.data_consegna ? `<div class="ocv2-item-meta">📅 Consegna: ${new Date(d.data_consegna).toLocaleDateString('it-IT')}${parseFloat(d.supplemento_express||0)>0?` · +€${parseFloat(d.supplemento_express).toFixed(2)} express`:''}</div>` : ''}
            <div class="ocv2-item-qty">Qtà: ${d.cantidad}</div>
          </div>
  ...
  const sideHtml = `
    <div class="ocv2-summary-title">Dettagli di pagamento</div>
    <div class="ocv2-summary-row"><span>Subtotale</span><span>€${prodTotale.toFixed(2)}</span></div>
    ...
    <div class="ocv2">
      <div class="ocv2-header">
        <div class="ocv2-header-left">
          ...
          <div class="ocv2-date">${new Date(o.createdAt).toLocaleDateString('it-IT', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })}</div>
        </div>
      ...
      <div class="ocv2-body">
        <div class="ocv2-main">
          ${stepperHtml}
          <div class="ocv2-items-title">Articoli Ordinati e Dettagli della Consegna</div>
          ${itemsHtml}
          ${trackingHtml}
        </div>
